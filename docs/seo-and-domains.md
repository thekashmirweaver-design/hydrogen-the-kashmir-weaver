...
| `www.thekashmirweaver.shop` | Alias | 301 → apex `.shop` (Hydrogen `server.ts`) |
| `https://thekashmirweaver.in` | Legacy / alternate TLD | 301 → `.shop` (Hydrogen `server.ts`) |
| `www.thekashmirweaver.in` | Alias | 301 → `.shop` |
| `https://thekashmirweaver.com` | Legacy app (Vercel) today; **Phase 2:** redirect → `.shop` while DNS stays on old app; **Phase 3:** DNS → Hydrogen |
...
Phase 1 (now)     thekashmirweaver.shop → Hydrogen (index here)
                  thekashmirweaver.in   → 301 → .shop (this repo)
Phase 2 (later)   thekashmirweaver.com  → still old app, redirect users → .shop
...
- `thekashmirweaver.in` (apex)
- `www.thekashmirweaver.in`

Both `.shop` and `.in` must be **connected** to the Hydrogen production environment in Shopify Admin so traffic reaches Oxygen.
...
```bash
curl -sI "https://thekashmirweaver.in/products/example"
# Expect: HTTP/1.1 301 → https://thekashmirweaver.shop/products/example
```
...
const PRIMARY_ORIGIN = 'https://thekashmirweaver.shop';
...
**Hydrogen:** No changes beyond Phase 1. Still canonical/index on `.shop`.
...
3. Add `.com` to `REDIRECT_TO_PRIMARY_HOSTS` in `app/lib/store-host-redirect.ts` if `.com` should 301 → `.shop`, **or** flip primary to `.com` and update `PUBLIC_STORE_URL` if `.com` becomes canonical.
...
| Fallback | `DEFAULT_STORE_URL` in `seo.ts` (`https://thekashmirweaver.shop`) |
| Host redirects | `app/lib/store-host-redirect.ts` — 301 `.in` / `www.*` → primary |
...
### `.shop` cutover checklist (Phase 1)

- [ ] Connect `thekashmirweaver.shop` in Hydrogen Production domains; set **Primary**
- [ ] Connect `thekashmirweaver.in` (redirect still needs DNS → Shopify/Oxygen)
- [ ] `PUBLIC_STORE_URL=https://thekashmirweaver.shop` on Oxygen + redeploy
- [ ] Update Customer Account API OAuth URIs for `.shop`
- [ ] GSC: add `thekashmirweaver.shop`, submit sitemap
- [ ] `curl -sI https://thekashmirweaver.in/` → 301 to `.shop`
...
No. Index **`.shop` only**. `.in` 301s to `.shop` so Google consolidates on one origin.

**Where is the `.in` → `.shop` redirect implemented?**  
