...
const CODE = readFileSync(
  resolve(ROOT, 'scripts/meta-pixel-shopify-custom-pixel.js'),
  'utf8',
)
  // Strip the big header comment for the editor paste.
  .replace(/^\/\*[\s\S]*?\*\//, '')
  .trim();

const STORE = '70yuey-sr';
const URL = `https://admin.shopify.com/store/${STORE}/settings/customer_events`;
const NAME = 'Meta Pixel — Purchase (Hydrogen)';
...
async function main() {
  ...
  if (/accounts\.shopify\.com|\/login/i.test(page.url())) {
    console.log('Login required — complete it in the browser (up to 2 min)…');
    await page.waitForURL(new RegExp(`/store/${STORE}/`), {timeout: 120000});
    await page.goto(URL, {waitUntil: 'domcontentloaded', timeout: 120000});
  ...
  await page.screenshot({
    path: resolve(ROOT, 'tmp/customer-events-1.png'),
    fullPage: true,
  });

  // If pixel already listed, open it for update.
  ...
  if (await existing.isVisible().catch(() => false)) {
    ...
    if (await add.first().isVisible().catch(() => false)) {
      ...
      // Some Admin UIs use a menu: Add pixel → Custom pixel
  ...
  await page.screenshot({
    path: resolve(ROOT, 'tmp/customer-events-2.png'),
    fullPage: true,
  });
  ...
  await context.storageState({path: STATE});
  await page.screenshot({
    path: resolve(ROOT, 'tmp/customer-events-done.png'),
    fullPage: true,
  });
  console.log('Screenshots in tmp/. Closing in 3s…');
